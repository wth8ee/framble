...
import { PlinkoMenu } from "@/components/plinko/PlinkoMenu";
import { PlinkoField } from "@/components/plinko/PlinkoField";
import { ScrollArea } from "@/components/ui/scroll-area";

export default function PlinkoPage() {
  const plinkoState = usePlinko();
  ...
  return (
    <ScrollArea className="h-[calc(100dvh-4rem)] w-full bg-slate-950 text-slate-50 selection:bg-emerald-500 selection:text-slate-950">
      <div className="min-h-[calc(100dvh-4rem)] py-6 pb-20 px-4 sm:px-6 lg:px-8 flex items-start md:items-center justify-center relative overflow-hidden w-full">
        ...
        <div className="w-full max-w-7xl bg-slate-900/40 border border-slate-900 rounded-xl p-4 md:p-6 backdrop-blur-md grid grid-cols-1 md:grid-cols-12 gap-6 z-10 my-auto">
          <PlinkoMenu {...plinkoState} />
          <PlinkoField {...plinkoState} />
        </div>
      </div>
    </ScrollArea>
  );
